...
$Id: FEL_DIARIO.java,v 1.2 2004/03/09 15:47:20 swhite Exp $
-----------------------------------------
$Author: swhite $
$Date: 2004/03/09 15:47:20 $
-----------------------------------------
*/
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class FEL_DIARIO {
	static Logger logger = Logger.getLogger(FEL_DIARIO.class);
	...
	//private static final String SELECT_FROM = "SELECT ID_MENSAJE, ID_USUARIO, FECHA, MENSAJE FROM FEL_DIARIO";
	private static final String SELECT_FROM = "SELECT FEL_DIARIO.ID_MENSAJE, FEL_DIARIO.ID_USUARIO, FEL_DIARIO.FECHA, FEL_DIARIO.MENSAJE, FEL_USUARIO.NOMBRE, FEL_USUARIO.APELLIDO1, FEL_USUARIO.APELLIDO2 FROM FEL_DIARIO, FEL_USUARIO WHERE FEL_DIARIO.ID_USUARIO =  FEL_USUARIO.ID_USUARIO ";
	private static final String SELECT_ORDER = " ORDER BY FEL_DIARIO.FECHA DESC";
	...
	*/
	public Vector loadFromDB()
	{
		...
		if(vRtado != null && !vRtado.isEmpty()) {
			Vector vDiario = new Vector();
			
			//Loop, add to vector and return vector
			for(int i = 0; i < vRtado.size() ; i++){
				vDiario.add(setElement( (Vector)vRtado.elementAt(i) ));
			}
			return vDiario;
	 ...
	 * @return
	 */
	public FEL_DIARIO loadFromDB(int iMessageId){
		String SQL_STRING;
	...
	public void saveToDB() throws java.sql.SQLException
	{
		...
		sSQL = new StringBuffer("INSERT INTO FEL_DIARIO ");
	...
	public static int delete(int nId) throws java.sql.SQLException{
		String SQL_STRING;
		SQL_STRING = "DELETE FROM FEL_DIARIO WHERE ID_MENSAJE=?";
	 ...
	 * @return
	 */
	public FEL_DIARIO setElement(Vector vRow)
	{
		FEL_DIARIO objDIARIO = new FEL_DIARIO();
		
		if(vRow.elementAt(0)!=null)
			objDIARIO.setID_MENSAJE( Integer.parseInt(vRow.elementAt(0).toString().trim()) );
		if(vRow.elementAt(1)!=null)
			objDIARIO.setID_USUARIO( Integer.parseInt(vRow.elementAt(1).toString().trim()) );
		if(vRow.elementAt(2)!=null)
			try {
				objDIARIO.setFECHA(sdfBBDD.parse(vRow.elementAt(2).toString().trim()) );
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		if(vRow.elementAt(3)!=null)
			objDIARIO.setMENSAJE(vRow.elementAt(3).toString().trim());
		if(vRow.elementAt(4)!=null)
			objDIARIO.setNOMBRE_USUARIO(vRow.elementAt(4).toString().trim());
		if(vRow.elementAt(5)!=null)
			objDIARIO.setAPELLIDO1_USUARIO(vRow.elementAt(5).toString().trim());
		if(vRow.elementAt(6)!=null)
			objDIARIO.setAPELLIDO2_USUARIO(vRow.elementAt(6).toString().trim());	
